...
namespace WebOfPlanets
{
    public class PlanetConnection : MonoBehaviour
    {
        ...
        public void Init(Transform a, Transform b, ConnectionType type, PlanetCreator planetCreator, float lifespan = 0f, GameObject markerPrefab = null, float markerScale = 3f, float markerHeight = 3f, float thickness = 0.6f, Pose? markerPoseA = null, Pose? markerPoseB = null)
        {
            ...
            // u jedan materijal, a OnDestroy ga čisti točno jednom. Umjesto punog
            // Lit cilindra animirani "tok energije" (dorada vizuala, srpanj 2026.);
            // boja i dalje dolazi iz UpdateHealthColor kroz _BaseColor pa health
            // gradijent i flicker rade nepromijenjeno.
            _material = CreateBeamMaterial();
            _beamA.GetComponent<Renderer>().sharedMaterial = _material;
        ...
        // Shader živi u Art/Shaders/Resources da ga player build ne strippa:
        // Shader.Find bez asset-materijala u sceni inače ne preživi build (isti
        // razlog kao fallback lanac u VfxManager.CreateParticleMaterial). Ako ga
        // ipak nema, fallback je staro ponašanje — puni Lit cilindar u boji.
        private Material CreateBeamMaterial()
        {
            Shader shader = Shader.Find("WebOfPlanets/ConnectionBeam");
            if (shader == null)
                return _beamA.GetComponent<Renderer>().material;

            var mat = new Material(shader);

            // Tempo toka po tieru: jača veza = brži i gušći tok energije;
            // Ancient (početne hub veze) sporo, "prastaro" pulsiranje.
            float flowSpeed, bandDensity;
            switch (Type)
            {
                case ConnectionType.Strong: flowSpeed = 4.5f; bandDensity = 0.45f; break;
                case ConnectionType.Mid:    flowSpeed = 3.0f; bandDensity = 0.35f; break;
                case ConnectionType.Weak:   flowSpeed = 1.8f; bandDensity = 0.28f; break;
                default:                    flowSpeed = 1.2f; bandDensity = 0.20f; break;
            }
            mat.SetFloat("_FlowSpeed", flowSpeed);
            mat.SetFloat("_BandDensity", bandDensity);
            return mat;
        }
        ...
        public void ApplyDamage(float amount)
        {
            ...
            if (Health <= 0f)
            {
                ...
                // Totemi se raspadnu s dimom umjesto trenutnog nestanka (dorada
                // vizuala, srpanj 2026.). Detach prije Destroy(gameObject): djeca
                // su ovog transforma pa bi inače nestala s njim isti frame.
                DetachAndDisintegrate(_markerA);
                DetachAndDisintegrate(_markerB);
                Destroy(gameObject);
            }
        }

        private static void DetachAndDisintegrate(GameObject marker)
        {
            if (marker == null) return;
            marker.transform.SetParent(null, worldPositionStays: true);
            DisintegrationEffect.Play(marker, duration: 0.7f);
        }
